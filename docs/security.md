...
mcsource is a small open-source utility. This page explains the macOS security warning you may see, what Apple's code-signing process actually guarantees, what mcsource does and does not do on your system, and how to run it directly from source if you prefer not to trust a pre-built binary.

---

## The Gatekeeper warning

When you download mcsource from GitHub and try to open it, macOS may show:

> *"Apple could not verify 'mcsource' is free of malware that may harm your Mac or compromise your privacy."*

This warning appears because mcsource is not signed with a paid Apple Developer certificate. It does **not** mean the app was found to contain malware — it means macOS cannot verify the identity of who built it through Apple's paid programme.
...
The warning you see with mcsource is entirely about *identity* (unknown developer) — not about the code itself having been found dangerous.

---

## What mcsource actually does on your system

mcsource is as minimal as a macOS app can be. Here is the complete list of things it does:
...
| Save your settings | `UserDefaults` (`com.nielsemmer.mcsource`) | Remember names, icons, toggle states |
| Register as a login item | `SMAppService.mainApp.register()` | Launch at login (only if you enable it) |

That is the entire surface area. mcsource:
...
If you prefer not to run a pre-built binary at all, you can build and run mcsource entirely from source in a few commands. You need Xcode Command Line Tools (`xcode-select --install`) and nothing else.
...
```bash
./scripts/bundle.sh
# Produces dist/McSource.app — drag to /Applications
open dist/McSource.app
```
...
If you believe you have found a security issue or unexpected behaviour, please [open an issue](https://github.com/niels-emmer/mcsource/issues) on GitHub with as much detail as possible.
